...
class Tetromino:
   ...
   # Method for rotating the tetromino 90 degrees clock-wise on the game grid
   def rotate(self,game_grid):
      # check if the tetromino can be moved in the given direction by using the
      # can_be_moved method defined below
      if not (self.can_be_moved("down", game_grid) and self.can_be_moved("right",game_grid) and self.can_be_moved("left",game_grid)):
         return False  # tetromino cannot be moved in the given direction
      else:
         n = len(self.tile_matrix)  # n = number of rows = number of columns
         sum_x = 0
         sum_y = 0
         # to find the pivot coordinates of the tetromino
         for col in range(n):
           for row in range(n):
              if self.tile_matrix[col][row] != None:
               sum_y += self.tile_matrix[col][row].position.y
               sum_x += self.tile_matrix[col][row].position.x
         pivot_x = int(sum_x / 4)
         pivot_y = int(sum_y / 4)
         ...
         for col in range(n):
           for row in range(n):
             if ((self.tile_matrix[col][row] != None) and (game_grid.is_occupied(col,row) != None)):
               ...
               self.tile_matrix[col][row].position.y = temp_x + pivot_y - pivot_x
      return True
